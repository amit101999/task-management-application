...
const LoginPage: React.FC = () => {
  ...
  const handleGoogleLogin = async (data: any) => {
    console.log(data)
    if (data.code !== "") {
      const res = await axios.post(`${import.meta.env.VITE_BASE_URL}/api/user/login-with-google?code=${data.code}`, {},  // empty body, since you only send query param
      ...
      <div className="w-full max-w-sm sm:max-w-md lg:max-w-lg xl:max-w-xl">
        <Card className="backdrop-blur-sm bg-white/80 border-white/20 shadow-xl p-6 sm:p-8">
          ...
          <div className="text-center mb-6 sm:mb-8">
            ...
            <p className="text-sm sm:text-base text-gray-600">Sign in to your Project Tracker account</p>
          </div>
