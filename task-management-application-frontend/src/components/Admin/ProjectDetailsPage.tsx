...
const StatusBadge = ({ status }: any) => {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 px-2 sm:px-3 py-1 rounded-full text-xs font-medium border ${styles[status]}`}
    >
      {icons[status]}
...
const Avatar: React.FC<{
  ...
  return (
    <div className="relative">
      <div
        className={`${sizeStyles[size]} ${colors[colorIndex]} rounded-full flex items-center justify-center text-white font-medium flex-shrink-0`}
      >
        {initials}
      </div>
      {isOnline !== undefined && (
...
const FileIcon: React.FC<{ type: string }> = ({ type }) => {
  ...
  switch (type) {
    case "pdf":
      return (
        <div
          className={`${iconStyles} bg-red-100 text-red-600 rounded flex items-center justify-center text-xs font-bold`}
        >
          PDF
        </div>
      );
    case "docx":
      return (
        <div
          className={`${iconStyles} bg-blue-100 text-blue-600 rounded flex items-center justify-center text-xs font-bold`}
        >
          DOC
        </div>
      );
    ...
    case "jpg":
      return (
        <div
          className={`${iconStyles} bg-green-100 text-green-600 rounded flex items-center justify-center text-xs font-bold`}
        >
          IMG
        </div>
      );
    case "xlsx":
      return (
        <div
          className={`${iconStyles} bg-emerald-100 text-emerald-600 rounded flex items-center justify-center text-xs font-bold`}
        >
          XLS
        </div>
      );
...
export default function ProjectDetailsPage() {
  ...
  const progress = ((closedTask?.length ?? 0) / (tasks?.length || 1)) * 100;
  ...
  const [comments, setComments] = useState<Comment[]>([
    {
      ...
      user: "Sarah Chen",
      message:
        "Great progress on the authenticati module! The security review looks good.",
    ...
    },
    {
      ...
      user: "Mike Johnson",
      message:
        "Updated the database schema. Ready for the next sprint planning.",
  ...
  const activities: ActivityItem[] = [
    {
      id: "1",
      user: "Mike Johnson",
      action: 'completed task "User Authentication"',
      timestamp: "2 hours ago",
      avatar: "MJ",
    },
    {
            ...
            <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4 mb-4 md:mb-6">
              ...
              <div className="flex flex-col sm:flex-row gap-2 sm:gap-4 text-sm whitespace-nowrap">
                <div className="flex items-center gap-2 text-gray-600">
                  ...
                  <span>
                    Start: {selectedProjectId?.startDate?.split("T")[0]}
                  </span>
                </div>
                <div className="flex items-center gap-2 text-gray-600">
                  <Calendar className="w-4 h-4 flex-shrink-0" />
